...
def generate_solver(
    ...
    ik_solver = kdl.ChainIkSolverPos_LMA(
        chain, eps=1e-5, maxiter=500, eps_joints=1e-15, L=L
    )
...
def forward_kinematics(
    fk_solver, joints: np.ndarray, nb_joints: int
...
def inverse_kinematics(
    ...
    nb_joints: int,
    locked_joints: dict = None,  # e.g., {2: 0.0}
) -> Tuple[float, np.ndarray]:
    ...
    for i in range(nb_joints):
        if locked_joints and i in locked_joints:
            _q0[i] = locked_joints[i]  # Set fixed value
        else:
            _q0[i] = q0[i]
